...
static lv_display_t *bsp_display_lcd_init()
{
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .flags = {
            .sw_rotate = true,
            /* Both the draw and SW-rotation buffer must be DMA capable.
             * Otherwise SPI allocates a temporary TX copy on every flush. */
            .buff_dma = true,
    ...
    /* CO5300 is a QSPI panel, not an RGB framebuffer. The SPI port installs
     * on_color_trans_done and releases LVGL's buffer only after DMA completes.
     * The RGB path releases it immediately and registers callbacks against
     * the wrong panel implementation. */
