...
static void style_page(lv_obj_t *object) {
    lv_obj_set_size(object, lv_pct(100), lv_pct(100)); lv_obj_set_style_bg_color(object, lv_color_black(), 0);
...
static lv_obj_t *button(lv_obj_t *parent, const char *text, int y, void (*action)()) {
    lv_obj_t *item = lv_button_create(parent); lv_obj_set_size(item, 312, 44); lv_obj_align(item, LV_ALIGN_TOP_MID, 0, y);
...
static void open_display() {
    lv_obj_add_flag(settings, LV_OBJ_FLAG_HIDDEN); page = lv_obj_create(lv_screen_active()); style_page(page); label(page, "Affichage", 16, &lv_font_montserrat_24, CYAN);
    lv_obj_t *value = label(page, "Luminosité", 70, &lv_font_montserrat_20, 0xffffff);
    lv_obj_t *slider = lv_slider_create(page); lv_obj_set_size(slider, 280, 18); lv_obj_align(slider, LV_ALIGN_TOP_MID, 0, 116); lv_slider_set_range(slider, 10, 100); lv_slider_set_value(slider, 80, LV_ANIM_OFF);
    ...
    (void)value; button(page, "Retour", 286, back_to_settings);
}
...
static void open_eyes() {
    lv_obj_add_flag(settings, LV_OBJ_FLAG_HIDDEN); page = lv_obj_create(lv_screen_active()); style_page(page); label(page, "Expressions", 8, &lv_font_montserrat_24, CYAN);
    button(page, "Normal", 44, set_idle); button(page, "Heureux", 94, set_happy); button(page, "Triste", 144, set_sad); button(page, "Fâché", 194, set_angry); button(page, "Endormi", 244, set_sleepy); button(page, "Retour", 294, back_to_settings);
}
...
static void open_wifi() {
    lv_obj_add_flag(settings, LV_OBJ_FLAG_HIDDEN); page = lv_obj_create(lv_screen_active()); style_page(page); label(page, "Wi-Fi", 16, &lv_font_montserrat_24, CYAN);
    wifi_status = label(page, "Wi-Fi", 62, &lv_font_montserrat_20, 0xffffff);
    label(page, "Pour configurer : branche USB,", 116, &lv_font_montserrat_14, MUTED); label(page, "ouvre l'installateur Kage Eyes", 140, &lv_font_montserrat_14, MUTED); label(page, "dans Chrome ou Edge.", 164, &lv_font_montserrat_14, MUTED);
    button(page, "Oublier ce réseau", 218, forget_wifi); button(page, "Retour", 278, back_to_settings); lv_timer_create(update_wifi_label, 500, nullptr); update_wifi_label(nullptr);
}
static void open_update() { lv_obj_add_flag(settings, LV_OBJ_FLAG_HIDDEN); page = lv_obj_create(lv_screen_active()); style_page(page); label(page, "Mise à jour", 20, &lv_font_montserrat_24, CYAN); label(page, "La mise à jour OTA arrive", 108, &lv_font_montserrat_20, 0xffffff); label(page, "après validation du Wi-Fi.", 138, &lv_font_montserrat_14, MUTED); button(page, "Retour", 278, back_to_settings); }
static void open_sd() { lv_obj_add_flag(settings, LV_OBJ_FLAG_HIDDEN); page = lv_obj_create(lv_screen_active()); style_page(page); label(page, "Carte SD", 20, &lv_font_montserrat_24, CYAN); label(page, "Lecture des fichiers :", 108, &lv_font_montserrat_20, 0xffffff); label(page, "prévue pour la phase audio.", 138, &lv_font_montserrat_14, MUTED); button(page, "Retour", 278, back_to_settings); }
...
void app_ui_begin(void) {
    ...
    lv_obj_t *tap = lv_button_create(home); lv_obj_set_size(tap, lv_pct(100), lv_pct(100)); lv_obj_set_style_bg_opa(tap, LV_OPA_TRANSP, 0); lv_obj_set_style_border_width(tap, 0, 0); lv_obj_add_event_cb(tap, [](lv_event_t *) { next_eye(); }, LV_EVENT_CLICKED, nullptr);
    lv_obj_t *menu = lv_button_create(home); lv_obj_set_size(menu, 96, 40); lv_obj_align(menu, LV_ALIGN_TOP_RIGHT, -12, 12); lv_obj_set_style_bg_color(menu, lv_color_hex(0x06141c), 0); lv_obj_set_style_border_color(menu, lv_color_hex(CYAN), 0); lv_obj_set_style_border_width(menu, 1, 0); lv_obj_add_event_cb(menu, [](lv_event_t *) { show_settings(); }, LV_EVENT_CLICKED, nullptr); lv_obj_t *menu_label = lv_label_create(menu); lv_label_set_text(menu_label, "MENU"); lv_obj_set_style_text_font(menu_label, &lv_font_montserrat_16, 0); lv_obj_center(menu_label);
    settings = lv_obj_create(screen); style_page(settings); lv_obj_add_flag(settings, LV_OBJ_FLAG_HIDDEN); label(settings, "Réglages", 8, &lv_font_montserrat_24, CYAN);
    button(settings, "Wi-Fi", 44, open_wifi); button(settings, "Affichage", 94, open_display); button(settings, "Yeux", 144, open_eyes); button(settings, "Carte SD", 194, open_sd); button(settings, "Mise à jour", 244, open_update); button(settings, "Retour", 294, show_home);
}
